...
def DFSVisit(G,u,visited,parent,children,times,low,time,cnt=1): #cnt controls not to overflow stack
    if cnt==5000:
        return
    time[0]+=1
    times[u]=time[0]
    ...
    for v in G[u].neighbours.keys():
        if not visited[v]:
            ...
            parent[v]=u
            children[u].append(v)
            DFSVisit(G,v,visited,parent,children,times,low,time,cnt+1)
        elif v!=parent[u]:
            anc.append(v)
    min_u=len(G)+1
...
def build_new_graph(G,articulation_points):
    ...
    bound=len(temp_vertices)
    for i in range(n):
        if not visited[i]:
            temp_vertices.append([[],set()])
            possible_vertex=[None] #so that reference is passed
            set_normal_vertex(G,i,visited,articulation_points,temp_vertices,possible_vertex)
            while possible_vertex[0] is not None:
                set_normal_vertex(G,possible_vertex[0],visited,articulation_points,temp_vertices,[None])
    m=len(temp_vertices)
    ...
    set_components(Gr,bound)
    return Gr

def set_normal_vertex(G,u,visited,articulation_points,temp_vertices,possible_vertex,cnt=1): #cnt controls not to overflow stack
    ...
    for v in G[u].neighbours.keys():
        if not visited[v]:
            if cnt==5000:
                possible_vertex[0]=v
                return
            set_normal_vertex(G,v,visited,articulation_points,temp_vertices,possible_vertex,cnt+1)
...
def set_components(Gr,bound):
    n=len(Gr)
    for i in range(n):
        for vertex in Gr[i].neighbours.keys():
            if vertex<bound:
                Gr[i].component[vertex]=set()
                Gr[i].component[vertex].add(i)
                set_component(Gr[i].component[vertex],i,Gr,vertex)

def set_component(component,v,Gr,ap):
    for u in Gr[v].neighbours.keys():
        if u!=ap and u not in component:
            component.add(u)
            set_component(component,u,Gr,ap)
            ...
            if u in articulation_points:
                for v in G[u].neighbours.keys():
                    if v not in used_edges[u]:
                        c=G[u].neighbours[v]
                        if G[v].index in Gr[G[p].index].component[G[u].index]:
                            if relax(times,arches,v,c,u,0):
                                q.put((-arches[u],times[u]+c,v,u))
                                used_edges[u].add(v)
                                used_edges[v].add(u)
                        elif relax(times,arches,v,c,u,1):
